...
class CustomBenchmarkTest(unittest.TestCase):
    def test_test5(self):
        state_snuqs = run_snuqs('qasm2/custom_benchmark/test5.qasm')
        state_qiskit = run_qiskit('qasm2/custom_benchmark/test5.qasm')
        for x, y in zip(state_snuqs, state_qiskit):
            self.assertAlmostEqual(x, y)

    def test_test7(self):
        state_snuqs = run_snuqs('qasm2/custom_benchmark/test7.qasm')
        state_qiskit = run_qiskit('qasm2/custom_benchmark/test7.qasm')
        for x, y in zip(state_snuqs, state_qiskit):
            self.assertAlmostEqual(x, y)
