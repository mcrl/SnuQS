#include "simulator_multi_cuda.h"
...
SimulatorMultiCUDA::SimulatorMultiCUDA() {
...
void SimulatorMultiCUDA::init(Qop *qop, Qreg &qreg, Creg &creg) {
...
void SimulatorMultiCUDA::fini(Qop *qop, Qreg &qreg, Creg &creg) {
...
void SimulatorMultiCUDA::cond(Qop *qop, Qreg &qreg, Creg &creg) {
  int val = 0;
  int k = 0;
...
void SimulatorMultiCUDA::measure(Qop *qop, Qreg &qreg, Creg &creg) {
...
void SimulatorMultiCUDA::reset(Qop *qop, Qreg &qreg, Creg &creg) {
  Qreg::amp_t *buf = qreg.get_buf();
  int num_qubits = qreg.get_num_qubits();
  const std::vector<int> &qubits = qop->get_qubits();
...
void SimulatorMultiCUDA::barrier(Qop *qop, Qreg &qreg, Creg &creg) {
  /* Do nothing */
}

void SimulatorMultiCUDA::ugate(Qop *qop, Qreg &qreg, Creg &creg) {
  Qreg::device_amp_t *buf = qreg.get_device_buf(0);
  int num_qubits = qreg.get_num_qubits();
  const std::vector<int> &qubits = qop->get_qubits();
...
void SimulatorMultiCUDA::cxgate(Qop *qop, Qreg &qreg, Creg &creg) {
  Qreg::device_amp_t *buf = qreg.get_device_buf(0);
  int num_qubits = qreg.get_num_qubits();
  const std::vector<int> &qubits = qop->get_qubits();
...
void SimulatorMultiCUDA::run_op(Qop *qop, Qreg &qreg, Creg &creg) {
  switch (qop->get_type()) {
...
void SimulatorMultiCUDA::run(std::vector<Qop*> &circ, Qreg &qreg, Creg &creg) {
  std::cout << "[SnuQS] Running Multi-GPU Simulator...\n";
  ...
  for (int i = 0; i < num_gpus; ++i) {
    ...
    min_free = std::min(free, min_free);
  }

  for (int i = 0; i < num_gpus; ++i) {
    cudaSetDevice(i);
    Qreg::device_amp_t *device_buf;
    cudaMalloc(&device_buf, min_free);
    qreg.set_device_buf(device_buf, i);
  }
  ...
  for (int i = 0; i < num_gpus; ++i) {
    cudaSetDevice(i);
    cudaFree(qreg.get_device_buf(i));
  }
}
