...
SimulatorOMP::SimulatorOMP() {
}

SimulatorOMP::~SimulatorOMP() {
}

void SimulatorOMP::init(Qop *qop, Qreg &qreg, Creg &creg) {
...
void SimulatorOMP::fini(Qop *qop, Qreg &qreg, Creg &creg) {
  /* Do nothing */
}

void SimulatorOMP::cond(Qop *qop, Qreg &qreg, Creg &creg) {
  int val = 0;
  int k = 0;
...
void SimulatorOMP::measure(Qop *qop, Qreg &qreg, Creg &creg) {
  Qreg::amp_t *buf = qreg.get_buf();
  int num_qubits = qreg.get_num_qubits();
...
void SimulatorOMP::reset(Qop *qop, Qreg &qreg, Creg &creg) {
  Qreg::amp_t *buf = qreg.get_buf();
  int num_qubits = qreg.get_num_qubits();
  const std::vector<int> &qubits = qop->get_qubits();
...
void SimulatorOMP::barrier(Qop *qop, Qreg &qreg, Creg &creg) {
  /* Do nothing */
}

void SimulatorOMP::ugate(Qop *qop, Qreg &qreg, Creg &creg) {
  Qreg::amp_t *buf = qreg.get_buf();
  int num_qubits = qreg.get_num_qubits();
  const std::vector<int> &qubits = qop->get_qubits();
...
void SimulatorOMP::cxgate(Qop *qop, Qreg &qreg, Creg &creg) {
  Qreg::amp_t *buf = qreg.get_buf();
  int num_qubits = qreg.get_num_qubits();
  const std::vector<int> &qubits = qop->get_qubits();
...
void SimulatorOMP::run_op(Qop* qop, Qreg &qreg, Creg &creg) {
    switch (qop->get_type()) {
...
void SimulatorOMP::run(std::vector<Qop*> &circ, Qreg &qreg, Creg &creg) {
  std::cout << "[SnuQS] Running OMP Simulator...\n";
