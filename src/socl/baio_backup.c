...
//#define MAX_TRANSFER_SIZE (1ul << 29)
...
int baio_init(struct baio_handle *hdlr, size_t block_size, unsigned queue_depth, const char *config_file)
{
	int err;
	...
	hdlr->block_size_per_device = ROUND_UP(block_size, ALIGN_SIZE);
	hdlr->block_size = hdlr->block_size_per_device * hdlr->num_devices;
//	hdlr->queue_depth = queue_depth;
//	hdlr->block_size_per_device = ROUND_UP(block_size, ALIGN_SIZE * hdlr->num_devices) / (hdlr->num_devices);
//	hdlr->block_size = hdlr->block_size_per_device * hdlr->num_devices;

	printf("num_devices: %lu\n", hdlr->num_devices);
	printf("block_size: %lu\n", hdlr->block_size);
	printf("block_size_per_device: %lu\n", hdlr->block_size_per_device);
	return 0;
}
...
int get_dev(struct baio_handle *hdlr, size_t addr)
{
	int di = (addr / hdlr->block_size_per_device) % hdlr->num_devices;
	//int d = (di + (addr / hdlr->block_size)) % hdlr->num_devices;
	int d = (di + (addr / (1ul << 34))) % hdlr->num_devices;
	//printf("dev of %lu: %d\n", addr, d);
	return d;
}
...
int baio_queue_read(struct baio_handle *hdlr, void *buf, off_t size, off_t offset)
{
	...
	while (size > 0) {
		...
		dev_off = (offset / block_size) * block_size_per_device + (offset % block_size_per_device);
...
int baio_queue_write(struct baio_handle *hdlr, void *buf, off_t size, off_t offset)
{
	...
	while (size > 0) {
		...
		while (left_size > 0) {
			...
			printf("write to %d from 0x%lx by %lu at %lu\n", di, addr, transfer_size, dev_off);
