...
class SemanticChecker : public snuqasmBaseListener {
	public:
	...
	void checkIdentType(antlr4::tree::TerminalNode *, SymbolTable::SymbolType) const;


	size_t getExplistSize(snuqasmParser::ExplistContext *ctx) const;
	size_t getIdlistSize(snuqasmParser::IdlistContext *ctx) const;
	size_t getMixedlistSize(snuqasmParser::MixedlistContext *ctx) const;
	size_t getAnylistSize(snuqasmParser::AnylistContext *ctx) const;

	void checkUnitaryGateExp(snuqasmParser::ExpContext *) const;
	void checkUnitaryGateExplist(snuqasmParser::ExplistContext *) const;
	void checkUnitaryGateArg(snuqasmParser::ArgumentContext*) const;
	void checkUnitaryGateArgs(std::vector<snuqasmParser::ArgumentContext*> &&) const;
	void checkUnitaryGate(snuqasmParser::UnitaryOpContext *) const;
	void checkCustomGate(snuqasmParser::CustomOpContext *) const;

	void insertIdlistToSymtab(snuqasmParser::IdlistContext *idlist, SymbolTable::SymbolType type);

	void checkIdlist(snuqasmParser::IdlistContext *ctx) const;
	void checkMixedlist(snuqasmParser::MixedlistContext *ctx) const;
  ...
  virtual void visitTerminal(antlr4::tree::TerminalNode * ctx) override;
  virtual void visitErrorNode(antlr4::tree::ErrorNode * ctx) override;
};
