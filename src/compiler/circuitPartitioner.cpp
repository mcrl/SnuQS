...
insertSwapGates(
	...
	for (auto &&perm : decomposeCycles(inmem0)) {
		if (perm.size() == 2) {
			it = gates.insert(it, gateFactory("swap", perm.perm_, {})) + 1;
		} else {
			it = gates.insert(it, gateFactory("nswap", perm.perm_, {})) + 1;
		}
	}
	it = gates.insert(it, gateFactory("placeholder", std::vector<size_t>(block.perm_), {})) + 1;
...
std::vector<QuantumCircuit> CircuitPartitioner::partition(const QuantumCircuit &circ) {
	size_t nlocal = snuqs::MAX_INMEM;
	QuantumCircuit opt_circ = circ;
	sortGates(opt_circ);

	if (circ.num_qubits() <= snuqs::MAX_INMEM) {
	...
	std::cout << "Number of Sub-circuits: " << circs.size() << "\n";
