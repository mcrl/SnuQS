...
//	return (g->type() != Gate::Type::NSWAP) && (g->type() != Gate::Type::PLACEHOLDER);
	if (g->type() == Gate::Type::NSWAP) {
		if (g->qubits().size() >= gpu::SMEM_NELEM) {
			return false;
		}
	}
	return true;
...
Gate::shared_ptr makeBlockGate(const QuantumCircuit &circ, const std::set<size_t> &equbits, size_t s, size_t e) {
	...
	auto block_gate = gateFactory("block", std::vector<size_t>(equbits.begin(), equbits.end()), {});
	...
	for (; i < gates.size(); i++) {
		...
		if (mergable(gates[i])) {
			for (auto q : gates[i]->qubits()) {
				if (q < MAX_INMEM && q >= gpu::WARPSHIFT) {
					equbits.insert(q);
				}
			}
		...
		*/
		if (mergable(gates[i])) {
			...
			if (equbits.size() + to_be_added <= gpu::MAX_CACHE) {
				for (auto q : gates[i]->qubits()) {
					if (q < MAX_INMEM && q >= gpu::WARPSHIFT) {
						equbits.insert(q);
					}
