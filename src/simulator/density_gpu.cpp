...
void DensityGPUSimulator::init(size_t num_qubits) {
	std::cout << "Density GPU Simulator\n";
...
void DensityGPUSimulator::deinit() {
	auto s = std::chrono::system_clock::now();
...
void DensityGPUSimulator::run(const std::vector<QuantumCircuit> &circs) {

	std::cout << "Simulation start..\n";
	auto s = std::chrono::system_clock::now();
	simulate(circs[0], state_, d_state_, stream_, num_amps_);
	std::chrono::duration<double> sec = (std::chrono::system_clock::now() - s);
