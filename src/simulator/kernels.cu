...
__device__ void group_apply(amp_t *state, size_t start, size_t step, size_t
num_amps, GPUGate *gate, amp_t *amps);
...
__global__ void gate_unitary_l(amp_t *state, size_t num_amps, size_t tgt, F0 f0,
F1 f1)
{
...
template<typename F0, typename F1>
__global__ void gate_unitary(amp_t *state, size_t num_amps, size_t tgt, F0 f0,
F1 f1)
{
    size_t i = blockDim.x * blockIdx.x + threadIdx.x;
    size_t s = (1ul << tgt);
...
__global__ void gate_swap(amp_t *state, size_t num_amps, size_t tgt0, size_t
tgt1)
{
    if (tgt0 > tgt1) {
            size_t tmp = tgt0;
...
template<typename F0, typename F1>
__global__ void gate_controlled_unitary(amp_t *state, size_t num_amps, size_t
ctrl, size_t tgt, F0 f0, F1 f1)
{
    size_t i = blockDim.x * blockIdx.x + threadIdx.x;
...
template<typename F0, typename F1>
__global__ void gate_controlled_unitary_tgt0(amp_t *state, size_t num_amps,
size_t ctrl, size_t tgt, F0 f0, F1 f1)
{
    size_t i = blockDim.x * blockIdx.x + threadIdx.x;
    size_t tgt0 = (ctrl < tgt) ? ctrl : tgt;
    size_t tgt1 = (ctrl > tgt) ? ctrl : tgt;
...
template<typename F>
__global__ void gate_controlled_phase(amp_t *state, size_t num_amps, size_t
ctrl, size_t tgt, F f)
{
    size_t i = blockDim.x * blockIdx.x + threadIdx.x;
    size_t tgt0 = (ctrl < tgt) ? ctrl : tgt;
...
template<typename F0, typename F1>
__device__ void group_controlled_unitary(amp_t *state, size_t ctrl, size_t tgt,
F0 f0, F1 f1)
{
    size_t tgt0 = (ctrl < tgt) ? ctrl : tgt;
    size_t tgt1 = (ctrl > tgt) ? ctrl : tgt;
...
template<typename F>
__device__ void group_controlled_phase(amp_t *state, size_t ctrl, size_t tgt, F
f)
{
    size_t tgt0 = (ctrl < tgt) ? ctrl : tgt;
    size_t tgt1 = (ctrl > tgt) ? ctrl : tgt;
...
__device__ void group_u2gate(amp_t *state, GPUGate *gate) {
    ...
    amp_t coef1 = {((real_t)M_SQRT1_2) * -cos(lambda), ((real_t)M_SQRT1_2) *
-sin(lambda)}; amp_t coef2 = {((real_t)M_SQRT1_2) * cos(phi),
((real_t)M_SQRT1_2) * sin(phi)}; amp_t coef3 = {((real_t)M_SQRT1_2) *
cos(phi+lambda), ((real_t)M_SQRT1_2) * sin(phi+lambda)};
...
__device__ void group_swapgate(amp_t *state, GPUGate *gate) {
    size_t tgt0 = (gate->qubits[0] < gate->qubits[1]) ? gate->qubits[0] :
gate->qubits[1]; size_t tgt1 = (gate->qubits[0] > gate->qubits[1]) ?
gate->qubits[0] : gate->qubits[1]; size_t st0 = (1ul << tgt0); size_t st1 = (1ul
<< tgt1); for (size_t i = threadIdx.x; i < (SMEM_NELEM>>2); i += blockDim.x) {
            size_t ci = ((i >> tgt0) << (tgt0+1)) + (i & (st0-1));
            size_t idx = ((ci >> tgt1) << (tgt1+1)) + (ci & (st1-1));
...
__device__ void group_nswap(amp_t *state, GPUGate *gate) {
    for (size_t i = threadIdx.x; i < (SMEM_NELEM>>gate->nqubits); i +=
blockDim.x) { for (int j = 0; j < gate->nqubits; j++) { size_t tgt =
gate->perm[j]; size_t st = (1ul << tgt); i = ((i >> tgt) << (tgt+1)) + (i & (st
- 1));
            }
...
__device__ bool applyable(size_t start, size_t step, GPUGate *gate) {
    if (gate->type == Gate::Type::T) {
            ...
            return ((1ul << tgt) < SMEM_NELEM) || (((start + SMEM_NELEM - 1) &
(1ul << tgt)) != 0); } else if (gate->type == Gate::Type::CZ) { size_t st0 =
(1ul << max(gate->qubits[0], gate->qubits[1])); size_t st1 = (1ul <<
min(gate->qubits[0], gate->qubits[1])); size_t mask = (start + SMEM_NELEM-1);
...
void applyUnitary(
    ...
    //		kernel::gate_unitary_l<<<nblocks, nthreads, 0, s>>>(state,
num_amps, tgt, f0, f1);
    //	} else {
    size_t nthreads = NTHREADS;
    size_t nblocks = std::max(1ul, num_amps / NTHREADS / 2);
    kernel::gate_unitary<<<nblocks, nthreads, 0, s>>>(state, num_amps, tgt, f0,
f1);
    //	}
    gpu::checkLastError();
}
...
void applyPhase(
    ...
    if (tgt < MAX_INMEM) {
            size_t nthreads = NTHREADS;
            size_t nblocks = std::max(1ul, num_amps / NTHREADS / 2);
            kernel::gate_phase<<<nblocks, nthreads, 0, s>>> ((amp_t*)state,
num_amps, tgt, f); gpu::checkLastError(); } else { applyAllAmptitudes(state,
num_amps, s, f);
    }
}
...
void applyControlledUnitary(
    ...
    if (tgt == 0) {
            ...
            kernel::gate_controlled_unitary_tgt0<<<nblocks, nthreads, 2 *
nthreads * sizeof(amp_t), s>>>( state, num_amps, ctrl, tgt, f0, f1);
            gpu::checkLastError();
    } else {
            size_t nthreads = NTHREADS;
            size_t nblocks = std::max(1ul, num_amps / NTHREADS / 4);
...
void zerostate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { size_t nthreads
= NTHREADS; size_t nblocks = std::max(1ul, num_amps / NTHREADS);
    kernel::gate_zerostate<<<nblocks, nthreads, 0, s>>>(state);
    gpu::checkLastError();
}

void uniform(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { size_t nqubit =
tgts.size(); real_t val = 1.; for (size_t i = 0; i < nqubit; i++) { val *=
((real_t)M_SQRT1_2);
    }
...
void idgate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) {
    // Do nothing
}

void hgate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { real_t coef =
((real_t)M_SQRT1_2); auto f0 = [=] __host__ __device__ (amp_t a0, amp_t a1) {
            return (a0 + a1) * coef;
    };
...
void xgate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { auto f0 = [=]
__host__ __device__ (amp_t a0, amp_t a1) { return a1;
    };
...
void ygate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { amp_t coef = {0,
1}; auto f0 = [=] __host__ __device__ (amp_t a0, amp_t a1) { return -coef*a1;
    };
...
void zgate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { auto f = [=]
__host__ __device__ (amp_t a) { return -a;
    };

    applyPhase(state, num_amps, tgts[0], s, f);
}

void sxgate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { amp_t coef0 =
{0.5, 0.5}; amp_t coef1 = {0.5, -0.5};
...
void sygate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { amp_t coef =
{0.5, 0.5};
...
void sgate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { amp_t coef = {0,
1}; auto f = [=] __host__ __device__ (amp_t a) { return coef * a;
    };

    applyPhase(state, num_amps, tgts[0], s, f);
}

void sdggate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { auto f = [=]
__host__ __device__ (amp_t a) { amp_t coef = {0, -1}; return coef * a;
    };

    applyPhase(state, num_amps, tgts[0], s, f);
}

void tgate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { amp_t coef =
{((real_t)M_SQRT1_2), ((real_t)M_SQRT1_2)}; auto f = [=] __host__ __device__
(amp_t a) { return coef * a;
    };

    applyPhase(state, num_amps, tgts[0], s, f);
}

void tdggate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { amp_t coef =
{((real_t)M_SQRT1_2), -((real_t)M_SQRT1_2)}; auto f = [=] __host__ __device__
(amp_t a) { return coef * a;
    };
...
void rxgate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { real_t theta =
params[0]; amp_t coef0 = {cos(theta/2), 0}; amp_t coef1 = {0, -sin(theta/2)};
...
void rygate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { real_t theta =
params[0]; amp_t coef0 = {cos(theta/2), 0}; amp_t coef1 = {sin(theta/2), 0};
...
void rzgate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { real_t theta =
params[0]; amp_t coef0 = {cos(theta/2), -sin(theta/2)}; amp_t coef1 =
...
void u1gate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { real_t lambda =
params[0]; amp_t coef = {cos(lambda), sin(lambda)}; auto f = [=] __host__
__device__ (amp_t a) { return coef * a;
    };

    applyPhase(state, num_amps, tgts[0], s, f);
}

void u2gate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { real_t phi =
params[0]; real_t lambda = params[1]; amp_t coef0 = {((real_t)M_SQRT1_2), 0};
    amp_t coef1 = {((real_t)M_SQRT1_2) * -cos(lambda), ((real_t)M_SQRT1_2) *
-sin(lambda)}; amp_t coef2 = {((real_t)M_SQRT1_2) * cos(phi),
((real_t)M_SQRT1_2) * sin(phi)}; amp_t coef3 = {((real_t)M_SQRT1_2) *
cos(phi+lambda), ((real_t)M_SQRT1_2) * sin(phi+lambda)};
...
void u3gate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) {
...
void swapgate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { size_t nthreads
= NTHREADS; size_t nblocks = std::max(1ul, num_amps / NTHREADS / 4);
...
void cxgate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { auto f0 = [=]
__host__ __device__ (amp_t a0, amp_t a1) { return a1;
    };
    auto f1 = [=] __host__ __device__ (amp_t a0, amp_t a1) {
...
void cygate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { amp_t coef = {0,
1}; auto f0 = [=] __host__ __device__ (amp_t a0, amp_t a1) { return -coef*a1;
    };
...
void czgate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { auto f = [=]
__host__ __device__ (amp_t a) { return -a;
    };
...
void chgate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { real_t coef =
((real_t)M_SQRT1_2); auto f0 = [=] __host__ __device__ (amp_t a0, amp_t a1) {
            return (a0 + a1) * coef;
    };
...
void crxgate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { real_t theta =
params[0]; amp_t coef0 = {cos(theta/2), 0}; amp_t coef1 = {0, -sin(theta/2)};
...
void crygate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { real_t theta =
params[0]; amp_t coef0 = {cos(theta/2), 0}; amp_t coef1 = {sin(theta/2), 0};
...
void crzgate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { real_t theta =
params[0]; amp_t coef0 = {cos(theta/2), -sin(theta/2)}; amp_t coef1 =
{cos(theta/2), sin(theta/2)};
...
void cu1gate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { real_t lambda =
params[0]; amp_t coef = {cos(lambda), sin(lambda)}; auto f = [=] __host__
__device__ (amp_t a) { return coef * a;
    };
...
void cu2gate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { real_t phi =
params[0]; real_t lambda = params[1]; amp_t coef0 = {((real_t)M_SQRT1_2), 0};
    amp_t coef1 = {((real_t)M_SQRT1_2) * -cos(lambda), ((real_t)M_SQRT1_2) *
-sin(lambda)}; amp_t coef2 = {((real_t)M_SQRT1_2) * cos(phi),
...
void cu3gate(amp_t *state, size_t num_amps, cudaStream_t s, const
std::vector<size_t> &tgts, const std::vector<real_t> &params) { real_t theta =
params[0]; real_t phi = params[1]; real_t lambda = params[2]; amp_t coef0 =
{cos(theta/2), 0}; amp_t coef1 = {-cos(lambda)*sin(theta/2),
sin(lambda)*sin(theta/2)}; amp_t coef2 = {cos(phi)*sin(theta/2),
sin(phi)*sin(theta/2)}; amp_t coef3 = {cos(phi+lambda)*cos(theta/2),
sin(phi+lambda)*cos(theta/2)};
...
void blockgate(amp_t *state, size_t num_amps, cudaStream_t s, const GPUGate
*gate) { size_t nthreads = NTHREADS; size_t nblocks = std::max(1ul, num_amps /
SMEM_NELEM); kernel::gate_block<<<nblocks, nthreads, 0, s>>>(state, num_amps,
gate); gpu::checkLastError();
}

void nswapgate(amp_t *state, size_t num_amps, cudaStream_t s, size_t nqubits,
const GPUGate *gate) { size_t nthreads = NTHREADS; size_t nblocks =
std::max(1ul, (num_amps >> nqubits) / NTHREADS); kernel::gate_nswap<<<nblocks,
nthreads, 0, s>>>(state, num_amps, gate); gpu::checkLastError();
}
