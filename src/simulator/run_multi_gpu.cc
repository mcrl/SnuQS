...
template <typename T> std::shared_ptr<Buffer<T>> runMultiGPU(Circuit &_circ) {
  ...
  std::shared_ptr<Circuit> circ =
      transpileMultiGPU(_circ, num_qubits, num_devices);
...
#pragma omp parallel for num_threads(num_devices)
  for (int i = 0; i < num_devices; ++i) {
    ...
    if (i == 0) {
      QopImpl<T>::initZeroState(buffer->ptr(), num_states_per_device, {}, {});
    } else {
      api::memset(buffer->ptr(), 0,
                  sizeof(std::complex<T>) * num_states_per_device);
    }
