...
from .qasm_context import QasmContext, SymbolType, ParamType
...
class QasmSemanticChecker(QasmStage):
    ...
    def check_listarg_dims(self, listarg):
        ...
        for dim in dims:
            if dim != 1 and dim != mx:
                raise QasmException(
                    "Illegal argument dimensions ({dim} != {mx}).")

    def check_num_params(self, name, listexp, ctx=None):
        if self.ctx.get_num_params(name) != len(listexp):
            raise QasmException(
                f"Illegal invocation of gate ({name}).")
    ...
    def check_valid_exp(self, exp, qop=None, ctx=None):
        ...
        if exp.ID():
            if qop:
                raise QasmException(
                    f"Invalid use of identifer in parameters ({exp.ID().getText()}).")
                return False
    ...
    def enterVersion(self, ctx: QASMParser.VersionContext):
        ...
        if major != '2' or minor != '0':
            raise QasmException(
                f"OPENQASM version mismatch (Expected 2.0).")
    ...
    def enterQregDeclStatement(self, ctx: QASMParser.QregDeclStatementContext):
        if not self.isPositive(int(ctx.NNINTEGER().getText())):
            raise QasmException(
                f"dimension must be positive.")
    ...
    def exitGatedeclStatement(self, ctx: QASMParser.GatedeclStatementContext):
        ...
        self.ctx.insert_symbol(
            ctx.gatedecl().ID().getText(), SymbolType.GATE, ctx)
    ...
    def enterOpaqueStatement(self, ctx: QASMParser.OpaqueStatementContext):
        raise QasmException("Not supported.")
    ...
    def enterQopMeasure(self, ctx: QASMParser.QopMeasureContext):
        ...
        for i, arg in enumerate(listarg):
            if i == 0:
                ...
                self.check_creg_defined(arg.ID())
            self.check_arg_index(arg)
