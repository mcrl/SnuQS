...
class ZZ_Swap_QAOA:
    ...
    def __init__(self, num_qubits: int, backend = None) -> None:
        """
    ...
    def _gen_swap_network(self, gamma: float, beta: float) -> Circuit:
        circuit = Circuit()
    ...
    def _get_opt_angles(self) -> tuple[npt.NDArray[np.float_], float]:
        def f(params: npt.NDArray[np.float_]) -> float:
            ...
            raw_probs = get_ideal_counts(circ, self.backend)
    ...
    def score(self, counts: Mapping[str, float]) -> float:
        ...
        """
        # Reverse bitstring ordering due to SWAP network
        raw_probs = get_ideal_counts(self.circuit(), self.backend)
        ideal_counts = {
    ...
    def run(self, shots: int):
        ...
        print(f"ZZ Swap QAOA test score: {score}")
        return score